...
La densité $\pi(\theta)$ n'a pas obligatoirement à être une vraie densité, en autant que $\int_\Theta \mathcal L(\theta|x)\pi(\theta)d\theta<\infty$ (ceci certifie que la [[C - Distribution à postériori]] est valide, i.e. $\int \pi(\theta|x)d\theta=1$)
...
### Remarque
L'usage de lois à priori impropores est justifié si la loi la à postériori [[C - Distribution à postériori]] est propre (non impropre).
