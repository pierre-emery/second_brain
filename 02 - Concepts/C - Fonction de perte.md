...
- Il existe des fonctions de perte standards qui en général font l'affaire Voici quelques exemples de fonction de pertes
...
$$ \delta_\pi(x)= \frac{\int_{\Theta} \omega(\theta),\theta,L(\theta\mid x),\pi(\theta),d\theta} {\int_{\Theta} \omega(\theta),L(\theta\mid x),\pi(\theta),d\theta}. $$

Posons $\pi^*(\theta)\propto \pi(\theta)\omega(\theta)$, soit une nouvelle densité _a priori_ car $\omega(\theta)\ge 0$ pour tout $\theta$. Si
...
Il s’agit de l’[[C - Estimateur de Bayes]] lorsque $\pi^*(\theta)$ et la densité _a priori_ est

$$ L(\theta, a) = (\theta - a)^2. $$ ce qui est la [[C - Perte quadratique (MSE)]]
...
Dans un cadre ML pur (hors bayésien), la fonction de perte sert directement à définir le [[C - Risque]] et le [[C - Risque Empirique]] qu'on cherche à minimiser lors de l'entraînement.
